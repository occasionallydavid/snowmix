...
int COpenglVideo::ExecuteShape(u_int32_t shape_id, u_int32_t level, double scale_x, double scale_y, double scale_z)
{
	...
	while (p) {
		...
		switch (p->type) {
			...
			case SM_GL_LIGHT:
//PMM13
if (m_verbose>5) fprintf(stderr, "glLight(%d,%d,%f)\n", (int)p->p3.lighti, (int) p->p2.pname, (float) p->p1.light_param);
...
#endif
//PMM 13 SM_GL_OPER_LIGHT(p->p3.lighti, p->p2.pname, p->p1.light_param);
				if ((n = AddToShapeCommand(shape_id, type))) return n;
